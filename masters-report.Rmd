...
output: pdf_document
bibliography: [bibliography.bib, bibliography.json] # Uncomment when person == "Kat"
---
...
<!-- ZZQ: Sort of an odd place for a citation, but I feel it. -->
<!--[@tidyverse]-->

<!-- ZZQ: I like the info presented in the first two paragraphs, but I think leading with how we consider the occurrence of hate crimes, from the statistical perspective, seems a bit "abrupt". Try leading with (1) what hate crimes are and (2) Why it's worth investigating hate crimes since the rise of the American "Alt-Right" movement. This way, we get the readers familiar with the problem we intend to address and why it's important. You can follow this up with things like: Where did the data come from and why we opt to focus our attention on the PNW in the post 9-11 years and finally what the central question we hope to address and how [E.g. "Using hate crime data from the FBI, we implement a spatio-temporal version of a generalized additive model to investigate how the rise of the Alt-Right has impacted the occurrences of hate crimes in the PNW for the post 9-11 era"]. Try to keep the first paragraph more "general" in feel so that you introduce the problem/idea/question and then we'll follow these general statements up with specifics in the second paragraph. -->
<!-- ZZQ: In the second paragraph, consider focusing on presenting the reader with relevant information about the data. Where did it come from? Why, given its size, did you opt to focus on the PNW? How did you choose the "time window" to investigate. You can also talk about how the issues with the "locations" and talk a bit about how you resolved these issues with some slick geocoding. Essentially, keep this paragraph about the data and then lead this paragraph into statistical methods for treating the occurrence of hate crimes -->
<!-- ZZQ: In the third paragraph, we can introduce the reader to the idea that hate crimes can be thought of as a nonhomogenous point process (We should mention "why" we consider this to be the case, i.e. hate crimes occur at points where underlying factors [both observable and unobservable] can impact the distribution of where and when they occur). We can mention then that previous work on crime has shown the usefulness of self-exciting point processes, but that in our case this class of models doesn't necessarily make sense (and why). You can then describe how you intend to use a GAM with a cubic spline for time and a tensor product smoother in space to model the spatio-temporal occurrences. Again, keep this info sort of "high level" since we'll hit the committee with the details in the methods section -->
<!-- ZZQ: Overall, our goal with the intro is to ease the reader into the issue, give some high level details to keep them interested, and layout some idea of what they should expect. Keeping the different pieces in separate paragraphs makes it a bit easier for the reader to keep track of the ideas (rather than going back and forth between the first two paragraphs). And if you're finding that keeping the relevent info about, say, the data to one paragraph is too tough, break it up into two (But again, keep the ideas "connected". With a bit of reworking, I think this intro will be sick (The ideas are all there, I think we just need to order them slightly differently .... speaking of which, if at some point you don't like my suggestions, just ignore them (Though I'll try to keep the comments "good", I promise!) -->
Hate crimes are crimes which are motivated, in whole or in part, by the bias of the perpetrator. This bias can be against a person's perceived race, religion, gender, sexual orientation, or disability status. These crimes often result in the loss of life or limb, the destruction of personal property, or the intimidation of an entire community of people. One of the things that makes them unique is their obvious connection to the political climate of the society they occur in. Given the dramatic changes in the American political climate in a time that has seen the borderline normalization of white nationalism, it is of interest to observe how the patterns of these crimes have changed from the "post 9/11 era" to the years of Trump presidency. 
<!-- Hate crimes are considered as a nonhomogeneous point process, and the intensity function is estimated using a Generalized additive model. To accomodate the spatio-temporal nature of the data set, a tensor product smoother is used, using a thin plate regression spline for the spatial domain, and a cubic regression spline for the temporal domain. Due to the overabundance of data, the scope of the investigation was limited to the pacific northwest by filtering the data to be between -125 and -116 degrees of longitude, and between 40 and 49 degrees of latitude. One question of interest concerns the changing political climate of the United States. With the rise in prominence of the "Alt-Right", a loose group of far-right political extremists, has there been an increase in hate crimes in the Pacific Northwest? For this reason the "post 9-11" era was considered, using only the hate crime data from 2002-2019. -->

This project was inspired by a data set on hate crimes, collected by the FBI. The data were retrieved from the FBI's Crime Data Explorer website [@fbi]. The data set itself is 209,442 rows and 33 columns, dating from 1991 through 2019, from agencies in all 50 U.S. states, D.C.,  some United States territories such as Guam, and tribal lands. The FBI data set did not contain data on the specific locations of the crimes that were observed. A "location name" variable was recorded, but this did not include any sort of spatial information like GPS coordinates; instead this variable described the *type* of location where the crime occurred. Examples of locations observed by this variable include values such as "Residence/Home" and "Church/Synagogue/Temple/Mosque". However, there were a few variables that could be used to glean some information about where the crimes occurred; for each crime that was recorded in the data set, information about the public agency reporting the crimes. These variables were the public agency name, the public agency unit, the state name, the agency type, and the division name. The different agency types that reported the crimes were "City", "County", "Other State Agency", "University or College", "State Police", "Other", "Tribal", and "Federal". For each type of agency, the other variables describing the agency could be concatenated into a search string. This search string was then used as a search query using the Google Places API, with the assistance of a function wrapper provided in the R package `Googleway` [@googleway]. The search results from this process were saved, and the latitude and longitude coordinates were added to the hate crime data set. However, after this first round of reverse geocoding, there were several locations which had issues; either google had not found a location, or google had found two locations. To remedy this, the search strings for these problem locations were used as search queries for OpenStreetMaps using a wrapper from the `tmaptools` package [@tmaptools]. Because this process was extremely approximate, this limits us in the ways we can treat the data. If the Benton County Sheriff's office reported a hate crime, it would be impossible to say where within the county that crime occurred, but at a more coarse spatial resolution, the difference between a hate crime in Corvallis and a hate crime in Philomath might not be one that matters. 

Because the data set was so large, the scope was limited to the Pacific Northwest. Figure 1 shows the area considered in this paper, as well as comparisons between hate crime rates of Washington and Oregon, adjusted for state population size using population data from the census [@census].

<!-- ZZQ: Moved this here since it comes after figure 1 is first mentioned. -->
...
These hate crimes are events which happen in space and time, which makes them the sort of phenomenon that can be thought of as a spatio-temporal point process. It has been shown in the past that a marked Hawkes point process can be used to model crimes, with the crimes having the property of self-excitation [@park2019]. At the time of data retrieval, this self-exciting property of crimes was known, and was a part of the motivation behind investigating the data set. However, self exciting point processes such as the Epidemic Type Aftershock Sequence (ETAS) model are notoriously computationally intensive, and the hate crime data set wound up being extremely large. Furthermore, since the location of the crimes needed to be reverse-geocoded, the approximate nature of the location of each crime could easily lead to a misleading model. Since crimes from all over portland are all being reverse geocoded to the same location, there is good reason to suspect that the estimated triggering intensity of such a model would be biased for closer distances. For these reasons, the self-excitation assumption was dropped, and the scope of the project became modeling the observed counts of hate crimes for a given raster pixel of the spatial domain. 
...
One issue that GAMs run into is the *bias/variance trade-off*. The idea behind the bias/variance trade-off is stated as:
...
The authors of this statement [@geman1992] characterize this trade-off as being a choice between two models. One model, by not presuming too much about the structure of the underlying relationship, achieves low degrees of bias in estimation at the cost of being more variable. The other model avoids being too variable and thus avoids overfitting, at the cost of capturing too little information about the underlying relationship. 

<!-- ZZQ: Include a brief explanation about how the figure below demonstrates the bias variance trade off. Start with something like, "In Figure 2, we can see a visual representation of the bias/variance trade off. In the left most pane, we see ..." -->
<!-- ZZQ: Basically, any plot that you include should have some sort of comment about in the writing. -->
On the left of Figure 2, we see a LOESS smoother that has been defined with a span that is too small, leading to a model with high variance. On the right the span was made too big and the model has high bias. In the middle a good balance between bias and variance has been found with an intermediate span. 
...
Traditionally, the bias/variance trade-off was regarded as an immutable fact of modeling data generating processes, one which causes complex models to suffer from overfitting. However there has recently been some criticism of this [@neal2019], arguing that while it is accurate for some models (such as the one in the above figure), it is not as universal a problem as once thought.  Although this bias/variance trade-off can be a nuisance when training some machine learning models, it provides a natural motivation for a type of GAM smoother called a penalized regression spline. 

<!-- ZZQ: Going to comment this line out since I think it's just a "note" to yourself -->
...
\]
However, doing so produces an *interpolation spline* [@green1994]. This smooth curve will connect all of the points in our dataset together, but it will be a curve with extreme amount of variance, as shown in Figure 3.
<!-- ZZQ: "... as shown in Figure 3." -->
...
In terms of the bias/variance trade-off, interpolation splines lean all the way towards "low bias high variance" which produces curves like the one in Figure 3.
<!-- ZZQ: "which produces curves like the one in Figure 3" -->
These interpolation splines are not particularly helpful for estimating $g(\cdot)$ when there is sizable variability in the response, as is the case in Figure 3 and the observed hate crime counts of the FBI dataset. 
<!-- They "wiggle" back and forth too much, and when used to try and recover a "signal" by smoothing away noise, they can produce artifacts where the spline goes way outside the range of where points are being observed. To mitigate this, we can penalize the amount of "wiggling" that the curve does.  -->
We note that the more the curve wiggles back and forth, the more regions the curve spends with high curvature. Intuitively, we can penalize this by penalizing the square of second derivative, since we wish to penalize both strong positive curvature and strong negative curvature. The minimization problem then becomes: 
\[
...
\]
The goal of smoothing is to estimate the function $g(\cdot)$ given a set of n observations $y_i$ and $\textbf{x}_i$. For cubic regression splines, what we as data scientists and statisticians are doing is taking an algorithmic modeling approach [@breiman2001], and saying to ourselves "I may not be able to recover $g(\cdot)$ exactly, however, I can create a function that is *reasonably close* by fitting a cubic regression spline to these data". Very rarely do we believe that the coefficients of a cubic regression spline carry real world meaning; the purpose of the coefficients is to create a function which makes "*reasonable*" estimations based on a set of data. Thinking in this way, we may decide it is worth it to abstract one step further, moving away from the cubic spline structure and creating a smoother that is based purely on making the estimates as "reasonable" as possible. 
...
This function will penalize the minimization problem for functions which are too "wiggly" over a d-dimensional space. In the case where d=1 the penalty (\ref{wiggly}) becomes $J(f) = \int_\mathbb{R} [f''(x)]^2dx$ and the resulting solution to (\ref{tps1}) is the *natural cubic spline* that we saw in the previous section is achieved [@fang2015].
...
Wood goes into more deail of how the R package `mgcv` minimizes this equation under the constraint [@wood2017], but the important part to recognize is that once the $\text{\boldmath$\delta$}_k$ is estimated, the full $\text{\boldmath$\delta$}$ is retrieved by a single matrix multiplication operation and the $\text{$\delta$}$ can be used with **E** to estimate \unboldmath$g(\cdot)$. 
...
Since $\lambda$ is chosen using the Generalized Cross Validation as an optimality criterion, all that remains is to specify the basis dimensions for the spatial and temporal marginal smooths. The question remains as to how these basis dimensions are to be specified. Initially, naively, the intuitive approach was to use AIC or GCV
<!-- ZZQ: Do we define "GCV" somewhere in the report? -->
<!-- yes we do, see equation (8) -->
as optimality criteria, and find a basis dimension that minimized one or both of them. However, fitting several models with varying basis dimensions showed no signs of achieving a minimum for either optimality criterion. Model fitting times were becoming untenable, taking as many as 8 hours to fit.
<!-- ZZQ: Give a rough estimate as to how long fitting times were taking. Was it on the order of seconds, minutes, hours? -->
It became clear that there was no reason to assume that either optimality criterion would achieve a minimum before the saturated model was fit. 

Fortunately, @wood2017 outlines a randomized diagnostic test to check if the basis dimensions are large enough, and he included an implementation of this test in `mgcv`. Since it was randomized, three replicates were taken for each combination of the values of spatial and temporal basis dimension were made to get an idea of the spread. This randomized test statistic is called the k-index; @mgcv suggests that models which produce a k-index above 1 will be sufficiently fit, and that models with a k-index below 1 might be underfit. GCV, AIC, and k-index are plotted in Figure 5 with both 5 and 7 for the temporal basis dimension, but the values are so similar that this may be difficult to see.
<!-- ZZQ: Include a brief bit of text describing the plot below. That is, "Figure 4 shows ...." -->

![GCV, AIC, and 3 k-index values were found for varying spatial and temporal basis dimensionality.](plots and figures/model_selection1.png)
...
These models (those in Figure 5) 
<!-- ZZQ: (Those shown in Figure 4) -->
were fit using a negative-binomial family for the error term of the model. It was assumed that this would be a better fit than a Poisson model, which can suffer from underdispersion. At this point, a Poisson model was fit using a spatial basis dimension of 300, and a temporal basis dimension of 7. This model, surprisingly, had a k-index that was consistently above 1, and with a higher percentage of deviance explained by the model than with the negative-binomial model. To be sure the satisfactory k-index was not a product of random chance, 100 k-index values were calculated, their box-and-whisker plot shown in figure 6.
<!-- ZZQ: Figure 5 shows... -->

![k-index values for the final model.](plots and figures/model_selection2.png)

The reason that this model selection procedure was not repeated is that these models take an extraordinary length of time to fit. The final model, using a Poisson family for the errors, with a spatial basis dimension of 300, and a temporal dimension of 7 took 8 hours to fit on a 1.8GHz processor with 16GB of memory. 
...
As is usually the case with residual analysis of GAMs and GLMs, residual analysis was a bit difficult, as can be seen in a plot of the deviance residuals (Figure 7). Fortunately the `mgcv` package includes a function for constructing quantile-quantile plots for GAM objects. These residuals show a fit that deviates from the reference line somewhat, but it is not too egregious, as seen in Figure 8.

![QQ plot of the residuals using `mgcv`::qq.gam()](plots and figures/residuals2.png)

A decent degree of temporal smoothing was achieved. In Portland, hate crimes were on the decline from 2009 to 2014, then took a sharp rise from 2015 onward. In Seattle, a steady upward trend in reported hate crimes was observed. These patterns can be seen in Figure 9 
...
In Figure 10 we can see the increase in the hate crime rate for Portland. Also of interest are several artifacts produced by model fitting; off the northwest coast of Oregon, near Astoria, a large patch of the pacific ocean has high predicted hate crime counts, but (obviously) no observed crimes. 
