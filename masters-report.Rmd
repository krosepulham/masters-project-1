...
![A plot of the residuals against the linear predictor of the points. Note that 7311 points were omitted with a linear predictor close to 0, observed values of zero, and a residual just below zero. These points likely occurred in raster pixels where there was no reason to expect hate crimes, but where the model was an overestimate due to zero not being a possible mean for a poisson distribution.](plots and figures/residuals1.png)
