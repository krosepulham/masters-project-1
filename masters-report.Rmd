...
Speaking extremely roughly, poisson processes are a family of mathematical models that are used to model 
*things*
<!-- ZZQ: Phenomena? -->
...
\]
However, doing so produces an *interpolation spline* [@green1994]. This smooth curve will connect all of the points in our dataset together, but it will be a curve with extreme amount of variance.
...
as optimality criteria, and find a basis dimension that minimized one or both of them. However, fitting several models with varying basis dimensions showed no signs of achieving a minimum for either optimality criterion. Model fitting times were becoming untenable,
<!-- ZZQ: Give a rough estimate as to how long fitting times were taking. Was it on the order of seconds, minutes, hours? -->
and it became clear that there was no reason to assume that either optimality criterion would achieve a minimum before the saturated model was fit. 
...
These models (those in the above figure) 
